...
const donationSchema = new mongoose.Schema({
  ...
  paymentMethod: {
    type: String,
    required: true,
    enum: ['cash', 'bank_transfer', 'mobile_money', 'online_payment', 'check', 'other']
  },
...
donationSchema.methods.processRecurringDonation = function() {
  if (this.isRecurring && this.recurringDetails) {
    ...
    switch (this.recurringDetails.frequency) {
      case 'weekly':
        this.recurringDetails.nextDueDate = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
        break;
      case 'monthly':
        this.recurringDetails.nextDueDate = new Date(now.getFullYear(), now.getMonth() + 1, now.getDate());
        break;
      case 'quarterly':
        this.recurringDetails.nextDueDate = new Date(now.getFullYear(), now.getMonth() + 3, now.getDate());
        break;
      case 'yearly':
        this.recurringDetails.nextDueDate = new Date(now.getFullYear() + 1, now.getMonth(), now.getDate());
        break;
    }
    
    // Check if recurring donation should end
    if (this.recurringDetails.endDate && this.recurringDetails.nextDueDate > this.recurringDetails.endDate) {
      this.isRecurring = false;
    }
    
    if (this.recurringDetails.totalOccurrences && this.recurringDetails.currentOccurrence >= this.recurringDetails.totalOccurrences) {
      this.isRecurring = false;
    }
  }
  return this.save();
